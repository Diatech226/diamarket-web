...
export default function ProductDetailPage() {
  ...
  return <div className='space-y-8'>
    <div className='grid gap-6 md:grid-cols-2'>
      <div><img src={resolveMediaUrl(product.images[selectedImage]) || '/placeholder.svg'} className='h-80 w-full rounded-2xl object-cover md:h-[440px]'/><div className='mt-3 flex gap-2'>{product.images.slice(0,4).map((img, i)=><button key={img+i} onClick={()=>setSelectedImage(i)} className={`overflow-hidden rounded-lg border ${selectedImage===i?'border-black':''}`}><img src={resolveMediaUrl(img)} className='h-16 w-16 object-cover'/></button>)}</div></div>
